...
textInput('xlab', 'X axis label', value = "") ,
    ...
    checkboxInput('identityline', 'Identity Line')       
        ...
        tabPanel("How To",
                 h5(" (fixed issue filter variables reset when a new data is uploaded)."),
                 h5("1. Upload your data file in CSV format."),
                 h5("2. It is assumed that your data is ready for ggplot (long format)."),                 
                 h5("3. x and y variables input allow numeric and factor variables."),
                 ...
                 h5("6. Filter variable 2 is applied after filter variable 1. Values shown for filter variable 2 will depend on your previous data exclusions via x/y sliders or filter variable 1"),

                 h5("7. If you want to change a numerical variable to categorical include it in the list of Categorical variables and then choose a number of cuts (default is 3). This helps when you want to group or color by cuts of a continuous variable."),
                 
                 h5("8. You can simply change a numeric variable to factor without binning in the treat as categories slot."),
                 
                 
                 h5("9. Download the plot using the options on the 'Download' tab. This section based on code from Mason DeCamillis ggplotLive app."),
                 h5("10. Visualize the data table in the 'Data' tab. You can reorder the columns, filter and much more."),
...
#actionButton("plotButton", "Update Plot"),
          ...
          uiOutput("optionsmenu") ,
                ...
                conditionalPanel(
                 ...
                 fluidRow(
                    ...
                    # column (12, h4("Choose plot types")),
                     column (3,
      ...
      conditionalPanel(
                 condition = "input.showfacets" , 
                     fluidRow(
                          # column (12, h4("Choose variables that map to Color, Fill, Group and Split by column or by Row (upload a dataset first)")),
                     
                     column (3, h4(""),uiOutput("pointsize"),uiOutput("fill")),
                     column (3, h4(""),uiOutput("colour"),uiOutput("group")),
                     column(3, h4(""),uiOutput("facet_col"),uiOutput("facet_row"))
                   )
                 ),
  ...
  df <-filedata()
    if (is.null(df)) return(NULL)
             ...
             fluidRow(
                      ...
                      column(6,checkboxInput('showplottypes', 'Plot types, Points, Lines (Options ?)', value = TRUE)),
                      column(6,checkboxInput('showfacets', 'Color Fill Group Split (Options ?)', value = TRUE) ),
...
output$pointsize <- renderUI({
    ...
    items=names(df)
    names(items)=items
    selectInput("pointsizein", "Pointsize:",    c("None",items)
)
...
p <- p + 
  ...
  stat_sum_df("median_hilow", geom = "smooth",conf.int=input$PI,col=mediancol)
   ...
   if(input$medianpoints)           
p <- p + 
      stat_sum_df("median_hilow", geom = "point",conf.int=input$PI,col=mediancol)
...
p <- p + 
  ...
  stat_sum_df("median_hilow", geom = "smooth",conf.int=input$PI,col=mediancol,aes(group=NULL))
   ...
   if(input$medianpoints)           
p <- p + 
      stat_sum_df("median_hilow", geom = "point",conf.int=input$PI,col=mediancol,aes(group=NULL))
